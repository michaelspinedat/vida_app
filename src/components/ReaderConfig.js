...
export default class ReaderConfig extends Component {
    ...
    state = {
        ...
        speechRate: 0.5,
        speechPitch: 1,
        text: "Esto es para probar el narrador"
    };
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                <Text style={styles.title}>{`Configuración narrador`}</Text>

                <Button title={`Leer`} onPress={this.readText} />

                <Text style={styles.label}>{`Estado: ${this.state.ttsStatus ||
                    ""}`}</Text>

                <Text style={styles.label}>{`Voz seleccionada: ${this.state
                    .selectedVoice || ""}`}</Text>

                <View style={styles.sliderContainer}>
                    <Text
                        style={styles.sliderLabel}
                    >{`Velocidad: ${this.state.speechRate.toFixed(2)}`}</Text>
                    <Slider
                        style={styles.slider}
                        minimumValue={0.01}
                        maximumValue={0.99}
                ...
                <View style={styles.sliderContainer}>
                    <Text
                        style={styles.sliderLabel}
                    >{`Tono: ${this.state.speechPitch.toFixed(2)}`}</Text>
                    <Slider
                        style={styles.slider}
                        minimumValue={0.5}
                        maximumValue={2}
